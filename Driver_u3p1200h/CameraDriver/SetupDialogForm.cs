...
namespace ASCOM.LeaderU3P1200H.Camera
{
    ...
    public partial class SetupDialogForm : Form
    {
        ...
        private void CmdOK_Click(object sender, EventArgs e) // OK button event handler
        {
            ...
            {
                CameraHardware.comPort = (string)comboBoxComPort.SelectedItem;
        ...
        private void InitUI()
        {
            ...
            if (comboBoxComPort.Items.Contains(CameraHardware.comPort))
            {
                comboBoxComPort.SelectedItem = CameraHardware.comPort;
            }
        ...
        {
            uint n = 0;
            dvpStatus status = DVPCamera.dvpRefresh(ref n);
            ...
            if (status == dvpStatus.DVP_STATUS_OK)
            {
                for (uint i = 0; i < n; i++)
                {
                    ...
                    comboBoxComPort.Items.Add(info.FriendlyName);
                    if (i == 0)
                        comboBoxComPort.Text = info.FriendlyName;
                }
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                DVPCamera.dvpShowPropertyModalDialog(CameraHardware.handle, this.Handle);
            }
        }

        private void button2_Click(object sender, EventArgs e)
        {
            var status = DVPCamera.dvpOpenByName(comboBoxComPort.Text, dvpOpenMode.OPEN_NORMAL, ref CameraHardware.handle);
            if (status == dvpStatus.DVP_STATUS_OK)
                Close();
        }
    }
