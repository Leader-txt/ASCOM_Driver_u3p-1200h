...
//
// ASCOM Camera driver for LeaderU3P1200H
//
...
namespace ASCOM.LeaderU3P1200H.Camera
{
    //
    ...
    //
    // Your driver's DeviceID is ASCOM.LeaderU3P1200H.Camera
    //
    // The COM Guid attribute sets the CLSID for ASCOM.LeaderU3P1200H.Camera
    // The COM ClassInterface/None attribute prevents an empty interface called _LeaderU3P1200H from being created and used as the [default] interface
    //
    ...
    /// </summary>
    [ComVisible(true)]
    [Guid("621b5714-530e-4c00-a30f-7e4adfb4b7e7")]
    [ProgId("ASCOM.LeaderU3P1200H.Camera")]
    [ServedClassName("ASCOM Camera Driver for LeaderU3P1200H")] // Driver description that appears in the Chooser, customise as required
    ...
    public class Camera : ReferenceCountedObjectBase, ICameraV3, IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="LeaderU3P1200H"/> class. Must be public to successfully register for COM.
        /// </summary>
        public Camera()
        {
            try
            {
                ...
                tl = new TraceLogger("", "LeaderU3P1200H.Driver"); // Remove the leading ASCOM. from the ProgId because this will be added back by TraceLogger.
                SetTraceState();
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.LeaderU3P1200H.Camera", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        public static int[,] ConvertBgr24ToIntArray(byte[] bgr24Data, int width, int height)
        {
            if (bgr24Data.Length != width * height * 3)
            {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    int blue = bgr24Data[index];
                    int green = bgr24Data[index + 1];
                    int red = bgr24Data[index + 2];

                    // Combine BGR channels into a single integer (BGRA format)
                    int pixelValue = (red << 16) | (green << 8) | blue;
                    //pixelValue = (red * 30 + green * 59 + blue * 11) / 100;
                    result[x, y] = pixelValue;

                    index += 3; // Move to the next pixel
                }
            }

            return result;
        }
        public static int[,,] Img2Arr(byte[] bgr24Data, int width, int height)
        {
            if (bgr24Data.Length != width * height * 3)
            {
                throw new ArgumentException("Invalid data length");
        ...
        static int[,] ConvertArray(int[,] source, int numx, int numy)
        {
        ...
        public bool ImageReady
        {
            get
            {
                ...
                /*if (image?.Length > 0)
                {
                    image[0, 0] = random.Next(int.MaxValue);
                    return true;
                }*/
                dvpFrameBuffer refRaw = new dvpFrameBuffer(), refOut = new dvpFrameBuffer();
                var status = DVPCamera.dvpGetFrameBuffer(CameraHardware.handle, ref refRaw, ref refOut, 4000);
                //bgr24
                //upleft
                byte[] data = new byte[refRaw.frame.uBytes];
                image = new int[refRaw.frame.iWidth, refRaw.frame.iHeight];
                Marshal.Copy(refRaw.pData, data, 0, data.Length);
                int index = 0;
                for (int y = 0; y < refRaw.frame.iHeight; y++)
                {
                    for (int x = 0; x < refRaw.frame.iWidth; x++)
                    {
                        image[x, y] = data[index]<2?0:data[index];
                        index++;
                    }
                }
                /*byte[] data = new byte[refOut.frame.uBytes];
                Marshal.Copy(refOut.pData, data, 0, data.Length);
                image = ConvertBgr24ToIntArray(data, refOut.frame.iWidth, refOut.frame.iHeight);*/
        ...
        public string LastExposureStartTime
        {
            get
            {
                return lastExposureTime.ToString("yyyy-MM-ddTHH:mm:ss");
                try
                {
        ...
        public double LastDuration = -1;
        public DateTime lastExposureTime = DateTime.MinValue;
        /// <summary>
